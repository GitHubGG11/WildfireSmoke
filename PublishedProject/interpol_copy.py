...
def returnInflow(pixels, time, map, dim, bounds, discret, factor=100000):
    ...
    for pixel in pixels:
        if pixel[0]/factor <= time:
            ...
            ele = 1
            items.append(math.stack([x, y, ele], dim=channel(vector='x,y,z')))
    return PointCloud(Point(math.stack(items, dim=spatial('points'))), bounds=bounds)

def returnInflowGrid(pixels, time, map, dim, bounds, discret, factor=100000):
    # Bounds should have a center at (0,0)
    ...
    for pixel in pixels:
        if pixel[0]/factor <= time:
            x, y = (((pixel[1][0]/dim[0]) + 0.5) * discret[0], ((pixel[1][1]/dim[1]) + 0.5) * discret[1])
            ele = 1
            items[int(ele)][int(y)][int(x)] += 0.1

    return math.stack([math.stack([math.stack(j, spatial('x')) for j in i], spatial('y')) for i in items], spatial('z'))
